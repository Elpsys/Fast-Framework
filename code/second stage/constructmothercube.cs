...
namespace gorubi
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            int termnum = 12;
            int TH_S = 10;
            int intercube_ds = 32;  //set the value of the degree of intermediate cube which was obtained from the first stage. If the procedure is terminated due to the situation out of the strategy, the value should be set the degree of the cube which is obtained in last iteration. 
            List<uint> curcube = new List<uint>() { 0, 1, 3, 5, 7, 10, 12, 14, 15, 17, 20, 22, 25, 27, 30, 32, 34, 37, 40, 42, 45, 47, 50, 53, 55, 60, 62, 68, 70, 73, 75, 79 };
            // 0, 3, 10, 12, 17, 19, 23, 28, 32, 34, 46, 51, 63, 66, 72, 78
            List<uint> remcube = new List<uint>() { };    //Remaining variables
            List<int> mds = new List<int>() { };   //all max degrees set
            int k = 0;
            int m = 0;
            int mini = 1000;
            int miniloc = 0;
            ...
            while (intercube_ds > 0)
            {
                ...
                Console.Write("\n****************** The " + (round + 205).ToString() + "-th round ******************\n");
                ...
                for (k = 0; k < remcube.Count; k++)
                {
                    StreamReader terms = new StreamReader("D:\\workspace\\C#work\\constructmcube\\terms.txt");
                    Console.Write("\n TEST  cube:");
                    ...
                    StreamWriter MaxDegT = new StreamWriter("D:\\workspace\\C#work\\constructmcube\\MaxdegreeTerms.txt", true);
                    MaxDegT.WriteLine("*****************round" + (round + 205) + "**********************\n");
                    for (int i = 0; i < curcube.Count; i++)
                    {
                ...
                if (mds.Min() == 0)
                {
                    if (intercube_ds > TH_S)
                    {
                        ...
                        if (mds.IndexOf(TH_S) == -1)
                        {
                            curcube.Add(remcube[mds.IndexOf(mds.Min())]);
                            intercube_ds = mds.Min();
                            Console.WriteLine("index" + mds.IndexOf(mds.Min()) + "        remcube" + remcube[mds.IndexOf(mds.Min())]);
                        }
                        else
                        {
                            curcube.Add(remcube[mds.IndexOf(TH_S)]);
                            intercube_ds = mds[mds.IndexOf(TH_S)];
                            Console.WriteLine("index" + mds.IndexOf(TH_S) + "        remcube" + remcube[mds.IndexOf(TH_S)]);
                        }
                    }
                    else
                    {
                        int IV1 = intercube_ds - 1;
                        int IV2 = intercube_ds - 2;
                        if (mds.IndexOf(IV1) != -1)
                        {
                            curcube.Add(remcube[mds.IndexOf(IV1)]);
                            intercube_ds = mds[mds.IndexOf(IV1)];
                            Console.WriteLine("index" + mds.IndexOf(IV1) + "        remcube" + remcube[mds.IndexOf(IV1)]);
                        }
                        else if (mds.IndexOf(IV2) != -1)
                        {
                            curcube.Add(remcube[mds.IndexOf(IV2)]);
                            intercube_ds = mds[mds.IndexOf(IV2)];
                            Console.WriteLine("index" + mds.IndexOf(IV2) + "        remcube" + remcube[mds.IndexOf(IV2)]);
                        }
                        else
                        {
                            int min = 0;
                            ...
                            for (int i = 0; i < mds.Count; i++)
                            {
                                if (intercube_ds - mds[i] >= 0)
                                {
                                    if (first)
                                    {
                                        min = intercube_ds - mds[i];
                                        indexmin = i;
                                        first = false;
                                    }
                                    if (intercube_ds - mds[i] < min)
                                    {
                                        min = intercube_ds - mds[i];
                                        indexmin = i;
                                    }
                            ...
                            curcube.Add(remcube[indexmin]);
                            intercube_ds = mds[indexmin];
                            Console.WriteLine("index" + indexmin + "        remcube" + remcube[indexmin]);
                        }
                    }
                }
                else
