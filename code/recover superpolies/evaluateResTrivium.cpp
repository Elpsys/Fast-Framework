...
{
    vector<string> files;
    cout << "进入函数：getJustCurrentFile" << endl;
    ...
    cout <<"退出函数：getJustCurrentFile" << endl;
    ...
    cout << "进入函数：clearMen" << endl;
    thread t(clearMem);
    cout << "退出函数：clearMen" << endl;
    vector<std::future<void>> futures;
    cout << "进入函数：readFileThread(调用线程执行)" << endl;
    for (auto& file : files)
    ...
    cout << "退出函数：readFileThread(获取线程所得结果)" << endl;
    t.detach();
    cout << "回收内存" << endl;
    malloc_trim(0);

    cout << "筛选满足条件的项，存入polyIndexMap" << endl ;
