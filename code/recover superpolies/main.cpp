...
void preCompute(int rounds, bitset<80>& cube)
{
	...
	triviumTrack(cube, nonZeroTrackReps);//返回nonZeroTrackReps，其中的元素是特定轮数Trivium的288个状态位信息


	vector<bitset<288>> varsTrackReps;
	triviumVarsTrack(cube, varsTrackReps, nonZeroTrackReps);//返回varsTrackReps，其中的元素是特定轮数Trivium的288个状态位信息

	bitset<288> nonZeroFlag, varsFlag;
	setRoundFlags(rounds, cube, nonZeroFlag, varsFlag, nonZeroTrackReps, varsTrackReps);//指定轮数r若小于nonZeroTrackReps、varsTrackReps的元素个数（Trivium已迭代轮数），令nonZeroFlag等
	// 于r轮nonZeroTrackReps中存储的状态位信息;varsFlag等于varsTrackReps中第r个元素的状态位信息。
	...
	for (int i = 0; i < 288; i++)
		if (nonZeroFlag[i] == 1 && varsFlag[i] == 0)
		{
			...
			conskCofs[i] = kcofs;    //选定cube和最后状态位的情况下，返回满足可分性质的超级多项式，存储在conskCofs里。
		}
...
int main()
{
	...
	map<int,vector<int>> cubeIndex;
	ifstream file("cube.txt"); // 打开文件
	if (file.is_open()) { // 检查文件是否成功打开
		...
		//cout << line << endl; // 输出每一行的内容
			...
			cubeIndex.insert(pair<int, vector<int>>(a,elements));
			a++;
		}
		for (auto i = cubeIndex.begin(); i != cubeIndex.end(); i++) {
			cout<<i->first<<endl;		
		}
	...
	for (auto i = cubeIndex.begin(); i != cubeIndex.end(); i++) {
		...
		string path = string("TERM/") + to_string(cubenumber) + string("res.txt");
		ofstream os;
		os.open(path, ios::out | ios::app);
		os << "************************************第" << cubenumber << "个************************************" << endl;
		os << "cube:"<<cube<<endl;
