...
namespace Evaluate_Degrees_For_many_cubes
{
    class Program
    {
        static void Main(string[] args)
        {
            int round = 610;
            int termnum = 36; 
            StreamReader manycubes = new StreamReader("D:\\workspace\\C#work\\ConsoleApp2\\manycubes.txt");

            for (int nn = 0; nn < 59; nn++)
            {
                ...
                int NO = 0;
                int curdeg = 0;
                StreamReader terms = new StreamReader("D:\\workspace\\C#work\\ConsoleApp2\\terms.txt");
                for (int m = 0; m < termnum; m++)
                {
                    ...
                    if (model.SolCount > 0)
                    {
                        ...
                        if (currentdeg > 80)     //Reject high-degree Cube  
                        {
                            break;
                        }
                        curdeg = maxdeg;
                ...
                if (maxdeg < 80)
                {
                    StreamWriter MaxDegT = new StreamWriter("D:\\workspace\\C#work\\ConsoleApp2\\MaxdegreeTerms.txt", true);
                    for (int i = 0; i < curcube.Count; i++)
                    {
                        if (i != curcube.Count - 1)
                        {
